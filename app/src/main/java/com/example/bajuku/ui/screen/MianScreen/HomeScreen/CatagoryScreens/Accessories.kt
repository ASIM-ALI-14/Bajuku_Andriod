...
@Composable
fun Accessories(onClick: (productId: String) -> Unit) {
    Column(
        modifier = Modifier
        ...
        CategoryRow("New Arrival", accessoriesProducts.take(2), onClick)
        verticalSpacingL()

        // --- Recommendation (Next 2 products) ---
        CategoryRow("Recommendation", accessoriesProducts.slice(2..3), onClick)
        verticalSpacingL()

        // --- Popular Jacket (Next 2 products) ---
        CategoryRow("Popular Jacket", accessoriesProducts.slice(4..5), onClick)
        verticalSpacingEXL()
    }
}
